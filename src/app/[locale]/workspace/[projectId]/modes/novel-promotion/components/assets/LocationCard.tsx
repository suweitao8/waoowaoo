...
export default function LocationCard({
  ...
  if (showSelectionMode) {
    ...
    return (
      <div className="col-span-3 pin-surface-elevated p-4 transition-all">
        <input
          ref={fileInputRef}
          type="file"
          accept="image/*"
    ...
    <>
      ...
      >
        {isGroupTaskRunning ? (
          ...
          <AppIcon name="refresh" className={`w-4 h-4 ${isTaskRunning ? 'text-white' : 'text-[var(--pin-text-secondary)]'}`} />
        )}
      </button>
  ...
  const compactHeaderActions = (
    <>
      ...
      {/* {onCopyFromGlobal && (
          ...
          className="flex-shrink-0 w-5 h-5 rounded hover:bg-[var(--pin-tone-info-bg)] flex items-center justify-center transition-colors"
          title={t('character.copyFromGlobal')}
        >
          <AppIcon name="arrowDownCircle" className="w-3.5 h-3.5 text-[var(--pin-tone-info-fg)]" />
        </button>
      )} */}
        <button
          onClick={onEdit}
  ...
  return (
    <div className="flex flex-col gap-2 pin-surface-elevated p-3">
      <input
        ref={fileInputRef}
        type="file"
        accept="image/*"
