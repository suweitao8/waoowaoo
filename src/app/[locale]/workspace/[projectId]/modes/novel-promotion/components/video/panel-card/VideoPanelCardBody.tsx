  ...
  return (
    <div className="p-4 space-y-2">
      <div className="flex items-center justify-between text-xs">
        <span className="px-2 py-0.5 bg-[var(--pin-tone-info-bg)] text-[var(--pin-tone-info-fg)] rounded font-medium">{panel.textPanel?.shot_type || t('panelCard.unknownShotType')}</span>
        {panel.textPanel?.duration && <span className="text-[var(--pin-text-tertiary)]">{panel.textPanel.duration}{t('promptModal.duration')}</span>}
      </div>

      <p className="text-sm text-[var(--pin-text-secondary)] line-clamp-2">{panel.textPanel?.description}</p>

      <div className="mt-3 pt-3 border-t border-[var(--pin-stroke-base)]">
        {(showsIncomingLinkBadge || showsOutgoingLinkBadge) && (
          <div className="mb-2 flex flex-wrap gap-1.5">
            {showsIncomingLinkBadge && (
              <span
                className={`inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-[11px] font-medium ${showsOutgoingLinkBadge
                    ...
                    : 'bg-[var(--pin-bg-muted)] text-[var(--pin-text-tertiary)] border border-[var(--pin-stroke-base)]'
                  }`}
              >
            ...
            {showsOutgoingLinkBadge && (
              <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-[11px] font-medium bg-[var(--pin-tone-info-bg)] text-[var(--pin-tone-info-fg)]">
        ...
        {showsPromptEditor && (
          <>
            <div className="flex items-center justify-between mb-1">
              <span className="text-xs font-medium text-[var(--pin-text-tertiary)]">{t('promptModal.promptLabel')}</span>
              {!promptEditor.isEditing && (
                <button onClick={promptEditor.handleStartEdit} className="text-[var(--pin-text-tertiary)] hover:text-[var(--pin-tone-info-fg)] transition-colors p-0.5">
            ...
            {promptEditor.isEditing ? (
              <div className="relative mb-3">
                <textarea
                  ...
                  autoFocus
                  className="w-full text-xs p-2 pr-16 border border-[var(--pin-stroke-focus)] rounded-lg bg-[var(--pin-bg-surface)] text-[var(--pin-text-secondary)] focus:outline-none focus:ring-1 focus:ring-[var(--pin-tone-info-fg)] resize-none"
                  rows={3}
                  placeholder={t('promptModal.placeholder')}
                />
                <div className="absolute right-1 top-1 flex flex-col gap-1">
                  <button onClick={promptEditor.handleSave} disabled={promptEditor.isSavingPrompt} className="px-2 py-1 text-[10px] bg-[var(--pin-color-brand)] text-white rounded">{promptEditor.isSavingPrompt ? '...' : t('panelCard.save')}</button>
                  <button onClick={promptEditor.handleCancelEdit} disabled={promptEditor.isSavingPrompt} className="px-2 py-1 text-[10px] bg-[var(--pin-bg-muted)] text-[var(--pin-text-secondary)] rounded">{t('panelCard.cancel')}</button>
                </div>
              </div>
            ) : (
              <div onClick={promptEditor.handleStartEdit} className="text-xs p-2 border border-[var(--pin-stroke-base)] rounded-lg bg-[var(--pin-bg-muted)] text-[var(--pin-text-secondary)] cursor-pointer">
                {promptEditor.localPrompt || <span className="text-[var(--pin-text-tertiary)] italic">{t('panelCard.clickToEditPrompt')}</span>}
              </div>
            )}
              ...
              return (
                <div className="mt-2 flex items-center gap-2">
                  <button
                    ...
                    className="flex-shrink-0 min-w-[120px] py-2 px-3 text-sm font-medium rounded-lg shadow-sm transition-all disabled:opacity-50 bg-[var(--pin-color-brand)] text-white"
                  >
              ...
              <>
                <div className="flex items-center gap-2">
                  <button
                    ...
                    className="flex-shrink-0 min-w-[90px] py-2 px-3 text-sm font-medium rounded-lg shadow-sm transition-all disabled:opacity-50 bg-[var(--pin-color-brand)] text-white"
                  >
                  ...
                  <div className="mt-2">
                    <div className="flex gap-2">
                      <button
                        ...
                        className="flex-1 py-1.5 text-xs rounded-lg transition-all flex items-center justify-center gap-1 bg-[var(--pin-color-brand)] text-white disabled:opacity-50"
                      >
                      ...
                      {(taskStatus.isLipSyncTaskRunning || panel.lipSyncVideoUrl) && voiceManager.hasMatchedAudio && (
                        <button onClick={lipSync.handleStartLipSync} disabled={lipSync.executingLipSync} className="flex-shrink-0 px-3 py-1.5 text-xs rounded-lg bg-[var(--pin-tone-warning-fg)] text-white">
                          {t('panelCard.redo')}
                        </button>
                      )}
                    </div>
                    ...
                    {voiceManager.audioGenerateError && (
                      <div className="mt-1 p-1.5 bg-[var(--pin-tone-danger-bg)] border border-[var(--pin-stroke-danger)] rounded text-[10px] text-[var(--pin-tone-danger-fg)]">
                        {voiceManager.audioGenerateError}
                      </div>
                    )}
                    ...
                    {voiceManager.localVoiceLines.length > 0 && (
                      <div className="mt-2 space-y-1">
                        {voiceManager.localVoiceLines.map((voiceLine) => {
                          ...
                          return (
                            <div key={voiceLine.id} className="flex items-start gap-1.5 p-1.5 bg-[var(--pin-bg-muted)] rounded text-[10px]">
                              {voiceLine.audioUrl ? (
                                <button
                                  onClick={(event) => {
                                  ...
                                  className="flex-shrink-0 w-5 h-5 rounded-full flex items-center justify-center transition-colors bg-[var(--pin-bg-muted)]"
                                ...
                                <button
                                  ...
                                  className="flex-shrink-0 px-1.5 py-0.5 bg-[var(--pin-color-brand)] text-white rounded disabled:opacity-50"
                              ...
                              <div className="flex-1 min-w-0">
                                <span className="text-[var(--pin-text-tertiary)]">{voiceLine.speaker}: </span>
                                <span className="text-[var(--pin-text-secondary)]">&ldquo;{voiceLine.content}&rdquo;</span>
                              </div>
                            </div>
                          )
                        })}
