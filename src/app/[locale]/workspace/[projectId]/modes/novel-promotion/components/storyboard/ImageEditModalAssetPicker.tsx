...
export default function ImageEditModalAssetPicker({
  ...
  return (
    <div className="fixed inset-0 pin-overlay z-[60] flex items-center justify-center p-4">
      <div className="pin-surface-modal w-full max-w-lg max-h-[80vh] overflow-hidden">
        <div className="p-4 border-b flex items-center justify-between">
          <h4 className="font-bold text-[var(--pin-text-primary)]">{t('imageEdit.selectAsset')}</h4>
          <button onClick={onClose} className="text-[var(--pin-text-tertiary)] hover:text-[var(--pin-text-secondary)]">
        ...
        <div className="p-4 overflow-y-auto max-h-[60vh]">
          {characters.length > 0 && (
            <div className="mb-4">
              <h5 className="text-sm font-medium text-[var(--pin-text-secondary)] mb-2 flex items-center gap-1.5">
                <AppIcon name="user" className="h-4 w-4 text-[var(--pin-text-tertiary)]" />
                  ...
                  return appearances.map((appearance) => {
                    ...
                    return (
                      <button
                        ...
                        className={`relative aspect-square rounded-lg overflow-hidden border-2 ${isSelected ? 'border-[var(--pin-stroke-focus)]' : 'border-transparent'}`}
                      >
                        {displayImageUrl ? (
                          ...
                          <div className="w-full h-full bg-[var(--pin-bg-muted)] flex items-center justify-center text-[var(--pin-text-tertiary)]">
                            <AppIcon name="user" className="h-7 w-7" />
                          </div>
                        )}
                        <div className="absolute bottom-0 left-0 right-0 bg-[var(--pin-overlay)] text-white text-xs p-1 truncate" title={displayName}>
                          {displayName}
                        </div>
                        {isSelected && (
                          <div className="absolute top-1 right-1 w-5 h-5 bg-[var(--pin-color-brand)] text-white rounded-full flex items-center justify-center">
          ...
          {locations.length > 0 && (
            <div>
              <h5 className="text-sm font-medium text-[var(--pin-text-secondary)] mb-2 flex items-center gap-1.5">
                <AppIcon name="imageAlt" className="h-4 w-4 text-[var(--pin-text-tertiary)]" />
                ...
                {locations.map((location) => {
                  ...
                  return (
                    <button
                      ...
                      className={`relative aspect-[3/2] rounded-lg overflow-hidden border-2 ${isSelected ? 'border-[var(--pin-stroke-focus)]' : 'border-transparent'}`}
                    >
                      {displayImageUrl ? (
                        <MediaImageWithLoading
                          src={displayImageUrl}
                      ...
                      <div className="absolute bottom-0 left-0 right-0 bg-[var(--pin-overlay)] text-white text-xs p-1 truncate">
                        {location.name}
                      </div>
                      {isSelected && (
                        <div className="absolute top-1 right-1 w-5 h-5 bg-[var(--pin-color-brand)] text-white rounded-full flex items-center justify-center">
        ...
        <div className="p-4 border-t flex justify-end">
          <button
            onClick={onClose}
            className="px-4 py-2 bg-[var(--pin-color-brand)] text-white rounded-lg hover:bg-[var(--pin-color-brand-hover)]"
          >
            {t('common.confirm')}
          </button>
        </div>
