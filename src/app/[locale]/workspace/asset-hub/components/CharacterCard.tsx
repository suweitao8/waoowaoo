...
export function CharacterCard({ character, onImageClick, onImageEdit, onVoiceDesign, onEdit, onVoiceSelect }: CharacterCardProps) {
    ...
    if (hasMultipleImages) {
        return (
            <div className="col-span-3 pin-surface p-4 relative">
                {/* 隐藏输入 */}
                ...
                <div className="flex items-center justify-between mb-4">
                    <div className="flex items-center gap-2">
                        <span className="text-sm font-semibold text-[var(--pin-text-primary)]">{character.name}</span>
                        <span className="pin-badge pin-badge-neutral px-2 py-0.5 text-xs">{appearance.changeReason}</span>
                        {isPrimaryAppearance ? (
                            <span className="pin-badge pin-badge-success px-2 py-0.5 text-xs">{tAssets('character.primary')}</span>
                        ) : (
                            <span className="pin-badge pin-badge-info px-2 py-0.5 text-xs">{tAssets('character.secondary')}</span>
                        )}
                    </div>
                    <div className="flex items-center gap-1">
                        <ImageGenerationInlineCountButton
                            prefix={isAppearanceTaskRunning ? (
                                <>
                                    ...
                                    <span className="text-[10px] font-medium text-[var(--pin-tone-info-fg)]">{tAssets('image.regenCountPrefix')}</span>
                                </>
                            ) : (
                                <>
                            ...
                            className="inline-flex h-6 items-center justify-center gap-1 rounded-md px-1.5 hover:bg-[var(--pin-tone-info-bg)] transition-colors disabled:opacity-50"
                        />
                        {hasPreviousVersion && (
                            <button onClick={handleUndo} className="pin-btn-base pin-btn-soft h-6 w-6 rounded-md" title={tAssets('image.undo')}>
                                <AppIcon name="sparkles" className="w-4 h-4 text-[var(--pin-tone-warning-fg)]" />
                            </button>
                        )}
                        <button onClick={(e) => {
                            e.stopPropagation()
                        ...
                        }} className="pin-btn-base pin-btn-soft h-6 w-6 rounded-md">
                ...
                {taskErrorDisplay && !isAppearanceTaskRunning && (
                    <div className="flex items-center gap-2 mb-3 p-2 rounded-lg bg-[var(--pin-danger-ring)] text-[var(--pin-tone-danger-fg)]">
                        ...
                        return (
                            <div key={index} className="relative group/thumb">
                                <div
                                    ...
                                    className={`rounded-lg overflow-hidden border-2 cursor-zoom-in transition-all ${isSelected ? 'border-[var(--pin-stroke-success)] ring-2 ring-[var(--pin-focus-ring)]' : 'border-[var(--pin-stroke-base)] hover:border-[var(--pin-stroke-focus)]'}`}
                                >
                                    ...
                                    />
                                    <div className={`absolute bottom-2 left-2 text-xs px-2 py-0.5 rounded ${isSelected ? 'pin-badge pin-badge-success' : 'pin-badge pin-badge-neutral'}`}>
                                        {tAssets('image.optionNumber', { number: index + 1 })}
                                    </div>
                                </div>
                                <button
                                    onClick={(e) => { e.stopPropagation(); handleSelectImage(isSelected ? null : index) }}
                                    className={`absolute top-2 right-2 pin-btn-base w-7 h-7 rounded-full flex items-center justify-center ${isSelected ? 'pin-btn-tone-success' : 'pin-btn-secondary'}`}
                                >
                ...
                {effectiveSelectedIndex !== null && (
                    <div className="mt-4 flex justify-end">
                        <button onClick={handleConfirmSelection} disabled={selectImage.isPending} className="pin-btn-base pin-btn-tone-success px-4 py-2 rounded-lg flex items-center gap-2 text-sm">
                ...
                {showDeleteMenu && appearanceCount > 1 && (
                    <>
                        ...
                        <div className="absolute right-4 top-12 z-20 pin-surface-modal py-1 min-w-[120px]">
                            <button onClick={handleDeleteAppearance} className="pin-btn-base pin-btn-soft w-full justify-start rounded-none px-3 py-1.5 text-left text-xs">{tAssets('image.deleteThis')}</button>
                            <button onClick={() => { setShowDeleteMenu(false); setShowDeleteConfirm(true) }} className="pin-btn-base pin-btn-soft w-full justify-start rounded-none px-3 py-1.5 text-left text-xs text-[var(--pin-tone-danger-fg)]">{tAssets('character.deleteWhole')}</button>
                        </div>
                    </>
                )}
                ...
                {/* 删除确认对话框 - 多图模式也需要 */}
                {showDeleteConfirm && (
                    <div className="fixed inset-0 pin-overlay flex items-center justify-center z-50">
                        <div className="pin-surface-modal p-4 m-4 max-w-sm">
                            <p className="mb-4 text-sm text-[var(--pin-text-primary)]">{t('confirmDeleteCharacter')}</p>
    ...
    // 单图模式
    return (
        <div className="pin-surface overflow-hidden relative group">
            <input ref={fileInputRef} type="file" accept="image/*" onChange={handleUpload} className="hidden" />
            <input ref={voiceInputRef} type="file" accept="audio/*" onChange={handleUploadVoice} className="hidden" />

            {/* 图片区域 */}
            <div className="relative aspect-[3/2] bg-[var(--pin-bg-muted)]">
                {displayImageUrl ? (
                    <>
                        <MediaImageWithLoading
                            src={displayImageUrl}
                        ...
                        {!isAppearanceTaskRunning && (
                            <div className="absolute top-2 left-2 flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                                <button onClick={() => fileInputRef.current?.click()} disabled={uploadImage.isPending} className="pin-btn-base pin-btn-secondary h-7 w-7 rounded-full">
                                    <AppIcon name="upload" className="w-4 h-4 text-[var(--pin-tone-success-fg)]" />
                                </button>
                                <button onClick={() => onImageEdit?.('character', character.id, character.name, effectiveSelectedIndex ?? 0, appearance.appearanceIndex)} className="pin-btn-base pin-btn-tone-info h-7 w-7 rounded-full">
                                    <AppIcon name="edit" className="w-4 h-4" />
                                </button>
                        <button onClick={() => handleGenerate()} className="pin-btn-base pin-btn-secondary h-7 w-7 rounded-full">
                                    <AppIcon name="refresh" className="w-4 h-4 text-[var(--pin-tone-info-fg)]" />
                                </button>
                                {hasPreviousVersion && (
                                    <button onClick={handleUndo} className="pin-btn-base pin-btn-secondary h-7 w-7 rounded-full">
                                        <AppIcon name="sparkles" className="w-4 h-4 text-[var(--pin-tone-warning-fg)]" />
                                    </button>
                                )}
                            </div>
                    ...
                    <div className="flex h-full flex-col items-center justify-center px-4 py-6 text-[var(--pin-text-tertiary)]">
                        ...
                        <ImageGenerationInlineCountButton
                            ...
                            className="pin-btn-base pin-btn-primary flex items-center gap-1.5 px-3 py-1.5 text-sm rounded-lg"
                ...
                {taskErrorDisplay && !isAppearanceTaskRunning && (
                    <div className="absolute inset-0 flex flex-col items-center justify-center bg-[var(--pin-danger-ring)] text-[var(--pin-tone-danger-fg)] p-3 gap-1">
            ...
            <div className="p-3">
                <div className="flex items-center justify-between">
                    <h3 className="font-medium text-[var(--pin-text-primary)] text-sm truncate">{character.name}</h3>
                    <div className="flex items-center gap-1">
                        {/* 编辑按钮 */}
                        <button
                            onClick={() => onEdit?.(character, appearance)}
                            className="pin-btn-base pin-btn-soft h-6 w-6 rounded-md opacity-0 group-hover:opacity-100"
                            title={tAssets('video.panelCard.editPrompt')}
                        >
                            <AppIcon name="edit" className="w-4 h-4 text-[var(--pin-text-secondary)]" />
                        </button>
                        {/* 删除按钮 */}
                        <button onClick={() => appearanceCount <= 1 ? setShowDeleteConfirm(true) : setShowDeleteMenu(!showDeleteMenu)} className="pin-btn-base pin-btn-soft h-6 w-6 rounded-md text-[var(--pin-tone-danger-fg)] opacity-0 group-hover:opacity-100">
                ...
                {appearanceCount > 1 && (
                    <div className="flex gap-1 mt-2 overflow-x-auto">
                        {character.appearances.map((app, index) => (
                            <button key={app.id} onClick={() => setActiveAppearance(index)} className={`pin-btn-base px-2 py-0.5 text-xs rounded-full whitespace-nowrap ${index === activeAppearance ? 'pin-btn-primary' : 'pin-btn-soft text-[var(--pin-text-secondary)]'}`}>
                ...
                {appearance?.description && <p className="mt-2 text-xs text-[var(--pin-text-secondary)] line-clamp-2">{appearance.description}</p>}
            ...
            {showDeleteConfirm && (
                <div className="absolute inset-0 pin-overlay flex items-center justify-center z-20">
                    <div className="pin-surface-modal p-4 m-4">
                        <p className="mb-4 text-sm text-[var(--pin-text-primary)]">{t('confirmDeleteCharacter')}</p>
                        <div className="flex gap-2 justify-end">
                            <button onClick={() => setShowDeleteConfirm(false)} className="pin-btn-base pin-btn-secondary px-3 py-1.5 rounded-lg text-sm">{t('cancel')}</button>
                            <button onClick={handleDelete} className="pin-btn-base pin-btn-danger px-3 py-1.5 rounded-lg text-sm">{t('delete')}</button>
            ...
            {showDeleteMenu && appearanceCount > 1 && (
                <>
                    <div className="fixed inset-0 z-10" onClick={() => setShowDeleteMenu(false)} />
                    <div className="absolute right-3 top-auto bottom-16 z-20 pin-surface-modal py-1 min-w-[120px]">
                        <button onClick={handleDeleteAppearance} className="pin-btn-base pin-btn-soft w-full justify-start rounded-none px-3 py-1.5 text-left text-xs">{tAssets('image.deleteThis')}</button>
                        <button onClick={() => { setShowDeleteMenu(false); setShowDeleteConfirm(true) }} className="pin-btn-base pin-btn-soft w-full justify-start rounded-none px-3 py-1.5 text-left text-xs text-[var(--pin-tone-danger-fg)]">{tAssets('character.deleteWhole')}</button>
                    </div>
                </>
