  ...
  return (
    <div className="px-4 sm:px-6 lg:px-10 pb-8 max-w-[1400px] mx-auto w-full">
      <div className="flex items-center justify-between mb-5">
        ...
        <button className="text-xs text-[var(--pin-tone-info-fg)] hover:underline font-medium">{t('viewAll')}</button>
      </div>
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-5 gap-4">
        {projects.map((p) => (
          <div key={p.id} className="pin-surface cursor-pointer group hover:border-[var(--pin-tone-info-fg)]/40 transition-all duration-300 overflow-hidden relative">
            ...
            <div className="p-5 relative z-10">
              ...
              <div className="flex items-start gap-2 mb-3">
                ...
                <p className="text-xs text-[var(--pin-text-secondary)] line-clamp-2 leading-relaxed">{p.description}</p>
              </div>
              <ProjectStats project={p} t={t} />
              <div className="flex items-center gap-1 mt-3 text-[10px] text-[var(--pin-text-tertiary)]">
  ...
  return (
    <div className="px-4 sm:px-6 lg:px-10 pb-8 max-w-[1400px] mx-auto w-full">
      ...
      <div className="flex gap-4 overflow-x-auto pb-4 scrollbar-hide snap-x snap-mandatory">
        {projects.map((p) => (
          <div key={p.id} className="min-w-[300px] max-w-[300px] snap-start pin-surface cursor-pointer group hover:border-[var(--pin-tone-info-fg)]/40 transition-all duration-300 overflow-hidden relative flex-shrink-0">
            <div className="absolute inset-0 rounded-[inherit] bg-gradient-to-br from-blue-500/5 to-purple-500/5 opacity-0 group-hover:opacity-100 transition-opacity duration-500 pointer-events-none" />
            <div className="p-5 relative z-10">
              <h3 className="text-base font-bold text-[var(--pin-text-primary)] mb-2 group-hover:text-[var(--pin-tone-info-fg)] transition-colors line-clamp-1">
                {p.name}
              </h3>
...
export function LayoutList({ projects, t }: { projects: MockProject[]; t: (key: string, params?: Record<string, string | number>) => string }) {
  return (
    <div className="px-4 sm:px-6 lg:px-10 pb-8 max-w-3xl mx-auto w-full">
      ...
      <div className="pin-surface overflow-hidden divide-y divide-[var(--pin-stroke-base)]">
        {projects.map((p) => (
          <div key={p.id} className="flex items-center gap-4 px-5 py-4 cursor-pointer group hover:bg-[var(--pin-bg-muted)] transition-colors">
            <div className="flex-1 min-w-0">
              <h3 className="text-sm font-bold text-[var(--pin-text-primary)] group-hover:text-[var(--pin-tone-info-fg)] transition-colors line-clamp-1 mb-0.5">
                {p.name}
              </h3>
              <p className="text-xs text-[var(--pin-text-tertiary)] line-clamp-1">{p.description}</p>
            </div>
            <div className="flex-shrink-0">
              <ProjectStats project={p} t={t} />
            </div>
            <div className="flex-shrink-0 flex items-center gap-1 text-[10px] text-[var(--pin-text-tertiary)]">
              <AppIcon name="clock" className="w-3 h-3" />
              {formatTimeAgo(p.updatedAt, t)}
            </div>
            <AppIcon name="chevronRight" className="w-4 h-4 text-[var(--pin-text-tertiary)] opacity-0 group-hover:opacity-100 transition-opacity flex-shrink-0" />
          </div>
        ))}
      </div>
    </div>
  ...
  return (
    <div className="px-4 sm:px-6 lg:px-10 pb-8 max-w-[1400px] mx-auto w-full">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
        {/* 大卡片 */}
        {first && (
          <div className="lg:col-span-1 pin-surface cursor-pointer group hover:border-[var(--pin-tone-info-fg)]/40 transition-all duration-300 overflow-hidden relative">
            <div className="absolute inset-0 rounded-[inherit] bg-gradient-to-br from-blue-500/5 to-purple-500/5 opacity-0 group-hover:opacity-100 transition-opacity duration-500 pointer-events-none" />
            <div className="p-6 relative z-10 flex flex-col justify-between h-full">
              <div>
                <div className="pin-badge pin-badge-info text-[10px] mb-3 w-fit">{t('latestUpdate')}</div>
                <h3 className="text-lg font-bold text-[var(--pin-text-primary)] mb-2 group-hover:text-[var(--pin-tone-info-fg)] transition-colors">
                  {first.name}
                </h3>
                <div className="flex items-start gap-2 mb-4">
                  <AppIcon name="fileText" className="w-3.5 h-3.5 text-[var(--pin-text-tertiary)] mt-0.5 flex-shrink-0" />
                  <p className="text-sm text-[var(--pin-text-secondary)] leading-relaxed">{first.description}</p>
                </div>
              </div>
              <div>
          ...
          {rest.map((p) => (
            <div key={p.id} className="pin-surface cursor-pointer group hover:border-[var(--pin-tone-info-fg)]/40 transition-all duration-300 overflow-hidden relative">
              ...
              <div className="p-4 relative z-10">
                <h3 className="text-sm font-bold text-[var(--pin-text-primary)] mb-1 group-hover:text-[var(--pin-tone-info-fg)] transition-colors line-clamp-1">
                  {p.name}
                </h3>
                <p className="text-xs text-[var(--pin-text-tertiary)] line-clamp-1 mb-3">{p.description}</p>
                <div className="flex items-center justify-between">
                  <ProjectStats project={p} t={t} />
                  <span className="text-[10px] text-[var(--pin-text-tertiary)]">{formatTimeAgo(p.updatedAt, t)}</span>
                </div>
              </div>
            </div>
          ))}
  ...
  return (
    <div className="px-4 pb-8 max-w-3xl mx-auto w-full">
      <div className="flex items-center justify-between mb-4">
        <h2 className="text-xs font-semibold text-[var(--pin-text-tertiary)] uppercase tracking-wider">{t('recentProjects')}</h2>
        <button className="text-xs text-[var(--pin-tone-info-fg)] hover:underline font-medium">{t('viewAll')}</button>
      </div>
      <div className="space-y-0.5">
        {projects.map((p) => (
          <div key={p.id} className="flex items-center justify-between py-3 px-4 rounded-xl hover:bg-[var(--pin-bg-muted)] cursor-pointer transition-colors group">
            <div className="flex items-center gap-3 min-w-0 flex-1">
              ...
              <span className="text-sm font-medium text-[var(--pin-text-primary)] group-hover:text-[var(--pin-tone-info-fg)] transition-colors truncate">{p.name}</span>
            </div>
            <div className="flex items-center gap-4 text-xs text-[var(--pin-text-tertiary)] flex-shrink-0">
