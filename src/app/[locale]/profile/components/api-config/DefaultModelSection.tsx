...
export function DefaultModelSection({
    ...
    return (
        <div className="pin-surface rounded-2xl p-5">
            <div className="flex items-center gap-2 mb-4">
                <span className="inline-flex h-7 w-7 items-center justify-center rounded-lg border border-[var(--pin-stroke-base)] bg-[var(--pin-bg-muted)] text-[var(--pin-text-secondary)]">
                    <AppIcon name="sparklesAlt" className="w-4 h-4" />
                </span>
                <h3 className="text-sm font-semibold text-[var(--pin-text-primary)]">{t('defaultModel.title')}</h3>
            </div>

            <p className="mb-4 text-xs text-[var(--pin-text-secondary)]">{t('defaultModel.hint')}</p>

            <div className="grid gap-3">
                {selectors.map(({ field, label }) => (
                    <div key={field} className="flex items-center gap-3">
                        <label className="w-24 shrink-0 text-sm text-[var(--pin-text-secondary)]">{label}</label>
                        <select
                            value={defaultModels[field as keyof typeof defaultModels] || ''}
                            onChange={(e) => onUpdateDefault(field, e.target.value)}
                            className="pin-select-base flex-1 px-3 py-2 text-sm"
                        >
